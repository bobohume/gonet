...
func getUpdateSql(p *Properties, classField reflect.StructField, classVal reflect.Value, sqlData *SqlData) (bool) {
...
func parseUpdateSql(obj interface{}, sqlData *SqlData){
	...
	for i := 0; i < classType.NumField(); i++{
		...
		bRight:= getUpdateSql(p, classType.Field(i), classVal.Field(i), sqlData)
		if !bRight{
...
func UpdateSqlEx(obj interface{}, sqltable string, params ...string) string {
	...
	for i := 0; i < classType.NumField(); i++ {
		...
		if exist || p.IsPrimary(){
			...
			bRight := getUpdateSql(p, sf, classVal.Field(i), sqlData)
			if !bRight{
