...
func (this *ServerMgr) Init() bool {
	...
	//初始化log文件
	this.m_Log.Init("game")
	//初始配置文件
	base.ReadConf("gonet.yaml", &CONF)

	ShowMessage := func() {
		this.m_Log.Println("**********************************************************")
		this.m_Log.Printf("\tGAME Version:\t%s", base.BUILD_NO)
		this.m_Log.Printf("\tGAME IP(LAN):\t%s:%d", CONF.Server.Ip, CONF.Server.Port)
	...
	this.m_pCluster.InitCluster(&common.ClusterInfo{Type: rpc.SERVICE_GAME, Ip: CONF.Server.Ip, Port: int32(CONF.Server.Port)}, CONF.Etcd.Endpoints, CONF.Nats.Endpoints)
...
func SendToClient(clusterId uint32, packet proto.Message) {
	...
	if pakcetHead != nil {
		SERVER.GetCluster().SendMsg(rpc.RpcHead{DestServerType: rpc.SERVICE_GATE, ClusterId: clusterId, Id: pakcetHead.Id}, "", proto.MessageName(packet), packet)
	}
}

//--------------发送给地图----------------------//
func SendToZone(Id int64, ClusterId uint32, funcName string, params ...interface{}) {
	head := rpc.RpcHead{Id: Id, ClusterId: ClusterId, DestServerType: rpc.SERVICE_ZONE, SrcClusterId: SERVER.GetCluster().Id()}
	SERVER.GetCluster().SendMsg(head, funcName, params...)
}
