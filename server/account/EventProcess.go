...
//创建账号
func (this *EventProcess) C_A_RegisterRequest(ctx context.Context, packet *message.C_A_RegisterRequest) {
	...
	if err == nil{
		...
		if !rs.Next(){
			...
			if (err == nil) {
				...
				//登录账号
				actor.MGR.SendMsg( rpc.RpcHead{},"Account_Login", accountName, accountId, socketId, this.GetRpcHead(ctx).SrcClusterId)
				nError = 0
			}
		}else{//账号存在
...
//登录账号
func (this *EventProcess) C_A_LoginRequest(ctx context.Context, packet *message.C_A_LoginRequest) {
	accountName := packet.GetAccountName()
	password := packet.GetPassword()
...
//创建玩家
func (this *EventProcess) W_A_CreatePlayer(ctx context.Context, accountId int64, playername string, sex int32, gClusterId uint32) {
...
//删除玩家
func (this *EventProcess) W_A_DeletePlayer(ctx context.Context, accountId int64, playerId int64) {
