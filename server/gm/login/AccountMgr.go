...
func (this *AccountMgr) AddAccount(accountId int64) *Account {
	LoadAccountDB := func(accountId int64) *AccountDB {
		rows, err := gm.SERVER.GetDB().Query(fmt.Sprintf("select account_id, account_name, status, login_time, logout_time, login_ip from tbl_account where account_id=%d", accountId))
...
func (this *AccountMgr) RemoveAccount(accountId int64) {
	...
	if pAccount != nil {
		...
		gm.SERVER.GetLog().Printf("账号[%d]断开链接", accountId)
	}
}
...
func (this *AccountMgr) Account_Login(ctx context.Context, accountName string, accountId int64, socketId uint32, id uint32, key int64) {
	...
	gm.SERVER.GetLog().Printf("帐号[%s]返回登录OK", accountName)
...
func (this *AccountMgr) CreatePlayerRequest(ctx context.Context, packet *message.CreatePlayerRequest) {
	...
	//查找账号玩家数量
	error := 0
	rows, err := gm.SERVER.GetDB().Query(fmt.Sprintf("select count(player_id) as player_count from tbl_player where account_id = %d", accountId))
	if err == nil {
		rs, err := orm.Query(rows, err)
		if err == nil && rs.Next() {
			player_count := rs.Row().Int("player_count")
			if player_count >= 1 { //创建玩家上限
				gm.SERVER.GetLog().Printf("账号[%d]创建玩家数量上限", accountId)
			} else { //创建玩家
				playerId := base.UUID.UUID()
				_, err = gm.SERVER.GetDB().Exec(fmt.Sprintf("insert into tbl_player (account_id, player_id, player_name, sex, level, gold, draw_gold)"+
					"values(%d, %d, '%s', %d, 1, 0,	0)", accountId, playerId, packet.PlayerName, packet.Sex))
				if err == nil {
					gm.SERVER.GetLog().Printf("账号[%d]创建玩家[%d]", accountId, playerId)
					//登录游戏
	...
	if error == 1 { //创建失败通知accout删除player
		gm.SERVER.GetLog().Printf("账号[%d]创建玩家失败", accountId)
	}
}
...
func (this *AccountMgr) LoginPlayerRequset(ctx context.Context, packet *message.LoginPlayerRequset) {
	...
	if pAccount != nil {
		if !pAccount.SetPlayerId(playerId) {
			gm.SERVER.GetLog().Printf("帐号[%d]登入的玩家[%d]不存在", accountId, playerId)
			return
		}

		pPlayer := gm.SERVER.GetPlayerRaft().GetPlayer(playerId)
		if pPlayer == nil {
			GClusterId := gm.SERVER.GetCluster().RandomCluster(rpc.RpcHead{Id: playerId, DestServerType: rpc.SERVICE_GAME}).ClusterId
			if GClusterId == 0 {
				gm.SERVER.GetLog().Println("没有可用的GAME集群")
				return
			}
			gm.SERVER.GetCluster().SendMsg(rpc.RpcHead{ClusterId: GClusterId, DestServerType: rpc.SERVICE_GAME}, "LoginPlayerRequset", playerId, head.SrcClusterId, pAccount.GateSocketId)
		}else{
			gm.SERVER.GetCluster().SendMsg(rpc.RpcHead{ClusterId: pPlayer.GClusterId, DestServerType: rpc.SERVICE_GAME}, "LoginPlayerRequset", playerId, head.SrcClusterId, pAccount.GateSocketId)
		}
	}
}
...
//账号断开连接
func (this *AccountMgr) Player_Lease_Expire(ctx context.Context) {
...
func LoadSimplePlayerDatas(accountId int64) []*model.SimplePlayerData{
	...
	rows, err := gm.SERVER.GetDB().Query(orm.LoadSql(pData,  orm.WithWhereStr(fmt.Sprintf("account_id=%d", accountId))))
	rs, err := orm.Query(rows, err)
	for rs.Next(){
