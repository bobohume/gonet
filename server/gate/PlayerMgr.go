...
type (
	...
	IPlayerMgr interface {
		...
		AddPlayerMap(uint32, rpc.PlayerClusterInfo) int
...
func (this *PlayerMgr) AddPlayerMap(socketId uint32, clusterInfo rpc.PlayerClusterInfo) int {
	playerId := clusterInfo.Id
	Id := this.GetSocket(playerId)
	this.ReleaseSocketMap(Id, Id != socketId)

	pPlayer := NewPlayer(socketId, playerId)
	pPlayer.GClusterId = clusterInfo.GClusterId
	pPlayer.ZClusterId = clusterInfo.ZClusterId
	this.m_Locker.Lock()
	this.m_PlayerMap[playerId] = pPlayer
	this.m_SocketMap[socketId] = playerId
	this.m_Locker.Unlock()
	SERVER.GetCluster().SendMsg(rpc.RpcHead{ClusterId: pPlayer.GClusterId, DestServerType: rpc.SERVICE_GAME, Id:playerId}, "Player_Login", SERVER.GetCluster().Id(), clusterInfo)
...
func (this *PlayerMgr) ADD_ACCOUNT(ctx context.Context, socketId uint32, clusterInfo rpc.PlayerClusterInfo) {
	SERVER.GetLog().Printf("login incoming  Socket:%d PlayerId:%d GClusterId:%d ", socketId, clusterInfo.Id, clusterInfo.GClusterId)
	this.AddPlayerMap(socketId, clusterInfo)
}

func (this *PlayerMgr) DEL_ACCOUNT(ctx context.Context, socketid uint32) {
	playerId := this.GetPlayerId(socketid)
	this.ReleaseSocketMap(socketid, true)
	SERVER.GetCluster().SendMsg(rpc.RpcHead{SendType: rpc.SEND_BOARD_CAST, DestServerType: rpc.SERVICE_GAME}, "G_ClientLost", playerId)
}
