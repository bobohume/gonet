...
type(
	Timer struct {
		...
		Flag [2]int64	`sql:"name:flag"`					//定时器数据
	...
	ITimerMgr interface {
		Init(int64)
		AddTimer(Id int, Flag [2]int64, ExpireTime int64)//添加定时器
...
func (this *TimerMgr) AddTimer(Id int, Flag [2]int64, ExpireTime int64){
	pTimer, bEx := this.m_TimerMap[Id]
	if bEx && pTimer != nil{
		pTimer.Flag = Flag
		pTimer.ExpireTime = ExpireTime
		this.m_db.Exec(db.UpdateSqlEx(pTimer, "tbl_timerset", "flag0", "flag1", "expire_time"))
	}else{
		pTimer = &Timer{}
		pTimer.Id = Id
