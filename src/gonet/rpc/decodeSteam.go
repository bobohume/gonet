...
func UnmarshalStream(bitstream *base.BitStream, funcName string, pFuncType reflect.Type) []interface{}{
	...
	for i := 0; i < nCurLen; i++  {
		...
		case RPC_BOOL_PTR_ARRAY_PTR:
			...
			tVal := reflect.ArrayOf(nLen, reflect.TypeOf(&aa))
			val := reflect.New(tVal)
			arrayPtr := uintptr(unsafe.Pointer(val.Elem().Addr().Pointer()))
			for i := 0; i < nLen; i++ {
				value :=  (**bool)(unsafe.Pointer(arrayPtr))
				arrayPtr = arrayPtr + SIZE_PTR
				val1 := bitstream.ReadFlag()
