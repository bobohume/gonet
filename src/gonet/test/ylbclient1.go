...
func sayhelloName(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/gm"{
		...
		if call == "restart" {
			{
				cmd := exec.Command("sh", "-c", "//root/swy/gameserver/scripts/stop.sh")
			...
			{
				cmd := exec.Command("sh", "-c", "//root/swy/gameserver/scripts/start.sh")
				cmd.Stdin = strings.NewReader("some input")
				var out bytes.Buffer
				cmd.Stdout = &out
				cmd.Run()
			...
			{
				cmd := exec.Command("ssh", "root@192.168.5.10", "cd /rot/swy/gameserver/ && svn up")
				cmd.Stdin = strings.NewReader("some input")
				var out bytes.Buffer
				cmd.Stdout = &out
				cmd.Run()
